...
// Nairobi regions with mock blood supply data
...
export default function RegionalHeatmapScreen({ navigation }) {
  const [selectedType, setSelectedType] = useState('O+');
  const [selectedRegion, setSelectedRegion] = useState(null);

  // Build markers for Leaflet map with supply-demand coloring
  const mapMarkers = NAIROBI_REGIONS.map((region) => {
    const supplyForType = region.supply[selectedType] || 0;
    let markerType = 'donor';
    let emoji = '✅';
  ...
  return (
    <ScreenContainer contentContainerStyle={styles.container}>
      ...
      {NAIROBI_REGIONS.sort((a, b) => (a.supply[selectedType] || 0) - (b.supply[selectedType] || 0))
        .map((region) => {
