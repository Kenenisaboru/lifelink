...
const DEMO_SCANNED_DONOR: ScannedDonorData = {
  donorId: 'donor-001',
  name: 'Sarah Connor',
  bloodType: 'O+',
  verified: true,
...
export default function QRCheckInScreen({ navigation }: QRCheckInScreenProps) {
  ...
  const [scannedDonor, setScannedDonor] = useState<ScannedDonorData | null>(null);
  const [scanState, setScanState] = useState<'idle' | 'scanning' | 'scanned' | 'confirmed' | 'error'>('idle');
