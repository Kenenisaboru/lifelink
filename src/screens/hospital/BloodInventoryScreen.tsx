...
import type { BloodInventoryScreenProps } from '../../types/navigation';
import type { BloodType } from '../../types';

export default function BloodInventoryScreen({ navigation }: BloodInventoryScreenProps) {
  const {
          ...
          <Card key={bt} style={styles.stockCard}>
            <View style={styles.stockRow}>
              ...
              <View style={styles.controlBtns}>
                <TouchableOpacity style={styles.ctrlBtn} onPress={() => updateStock(bt as BloodType, -1)}>
                  <Text style={styles.ctrlBtnText}>−</Text>
                </TouchableOpacity>
                <TouchableOpacity style={[styles.ctrlBtn, styles.addBtn]} onPress={() => updateStock(bt as BloodType, 1)}>
      ...
      <Card style={styles.chartCard}>
        <View style={styles.miniChart}>
          {stockHistory.map((day, idx) => {
            const val = typeof day['O+'] === 'number' ? day['O+'] : 5;
            const h = Math.min(80, (val / 15) * 80);
            return (
              <View key={idx} style={styles.chartCol}>
                <View style={[styles.chartBar, { height: h, backgroundColor: val <= 3 ? COLORS.primary : COLORS.secondary }]} />
