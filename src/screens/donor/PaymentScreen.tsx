...
import { holdFunds } from '../../services/EscrowService';
import {
  PAYMENT_METHODS,
...
export default function PaymentScreen({ route, navigation }: PaymentScreenProps) {
  ...
  const [selectedMethodId, setSelectedMethodId] = useState<PaymentGateway>('telebirr');
  ...
  const [successData, setSuccessData] = useState<DonorResponse | null>(null);
  ...
  const handleToggleCoverFull = (val: boolean) => {
  ...
  const handlePayment = async () => {
    ...
    const responseObj: DonorResponse = {
      donorId: user?.uid || 'donor-demo-123',
      donorName: user?.name || 'Sarah Connor',
      bloodType: donorUser?.bloodType || request.bloodType,
        ...
        <View style={styles.modalOverlay}>
          <Card style={styles.modalCard} variant="glow">
            ...
            <Text style={styles.modalSub}>
              Transport payment confirmed via {successData?.paymentMethod}. Funds held safely in escrow. Broadcast sent to {request.hospitalName}.
            </Text>
