...
import type { DonorDashboardScreenProps } from '../../types/navigation';
import type { BloodRequest, DonorUser, GeoLocation } from '../../types';

export default function DonorDashboardScreen({ navigation }: DonorDashboardScreenProps) {
  const { user, logout, toggleAvailability, updateProfile } = useAuthStore();
  ...
  const [userLocation, setUserLocation] = useState<GeoLocation>(
    donorUser?.location || { lat: -1.286389, lng: 36.817223, city: 'Nairobi CBD' }
  );
      ...
      try {
        ...
        try {
          ...
          if (address && (address.city || address.subregion || address.district)) {
            const cityName = address.city || address.subregion || address.district || 'Nairobi';
  ...
  const matchedAlerts = activeOpenRequests
    .map((req) => {
      ...
      const compatible = isBloodCompatible(donorUser?.bloodType, req.bloodType);
  ...
  const handleRespond = (request: BloodRequest, distanceKm?: number | null) => {
    navigation.navigate('Payment', { request, distanceKm: distanceKm || 2.4 });
  };
    ...
    >
      ...
      <Card style={styles.profileCard} variant="glow">
        <View style={styles.profileRow}>
          <View style={styles.bloodTypeBadgeContainer}>
            ...
            <Text style={styles.bloodTypeValue}>{donorUser?.bloodType || 'O+'}</Text>
          </View>
        ...
        <View style={styles.availabilityRow}>
          ...
          <Switch
            value={!!donorUser?.available}
            onValueChange={toggleAvailability}
            trackColor={{ false: COLORS.inputBorder, true: 'rgba(0, 230, 118, 0.4)' }}
            thumbColor={donorUser?.available ? COLORS.accentGreen : COLORS.textMuted}
          />
        </View>
      </Card>
      ...
      <View style={styles.sectionHeader}>
        <View>
          ...
          <Text style={styles.sectionSub}>Matching blood type ({donorUser?.bloodType || 'O+'})</Text>
        </View>
      ...
      {!donorUser?.available ? (
        ...
        <Card style={styles.emptyCard}>
          ...
          <Text style={styles.emptySub}>
            No active emergency blood requests match your blood type ({donorUser?.bloodType || 'O+'}) within {maxRadiusKm}km. Pull down to refresh.
          </Text>
        </Card>
      ) : (
